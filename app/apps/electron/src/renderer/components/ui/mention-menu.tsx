...
// S8: @ only for people/Agent/identity roster (type 'agent').
// Skill/Source/File bracket execution markers ONLY inserted by / (slash-command-menu).
// This component may still render other types if passed (compat), but @ triggers no longer produce them.

// ============================================================================
// Types
// ============================================================================

export type MentionItemType = 'agent' | 'skill' | 'source' | 'file' | 'folder' // agent for @ roster (people/Agent/identity). bracket types are parser-only for execution markers generated by / menu.
...
export function useInlineMention({
  ...
  // Build sections — S8: @ only for people/Agent/identity (roster via team projection).
  // Skills/Sources/Files are ONLY via / slash menu (slash-command-menu + InlineSlashCommand).
  // This hook no longer emits skill/source/file sections for @ triggers.
  const sections = React.useMemo((): MentionSection[] => {
    // Agent roster is built by caller (FreeFormInput team path) and passed directly to InlineMentionMenu.
    // General @ no longer populates skill/source/file.
    return []
  }, [])
  ...
  const handleInputChange = React.useCallback((value: string, cursorPosition: number) => {
    ...
    // S8: @ is ONLY for team roster (people/Agent/identity) in FreeFormInput's direct teamMention path.
    // This legacy hook (useInlineMention) must NEVER open on @ for skills/sources/files.
    // If an @ is typed here, close immediately (roster @ is handled outside this hook).
    ...
    if (isValidTrigger) {
      // Force close for any @ — do not populate skill/source/file. Roster uses separate team path.
      setIsOpen(false)
      setFilter('')
      setCommittedFilter('')
      setAtStart(-1)
    ...
    // Legacy non-@ trigger paths retained for callers that never used @.
    // / slash path drives skill/source/file via slash-command-menu; this hook no longer services @.
    const filterText = (atMatch && atMatch[1]) || ''
  ...
  const handleSelect = React.useCallback((item: MentionItem): { value: string; cursorPosition: number } => {
    ...
    if (atStart >= 0) {
      ...
      // S8: @ only inserts people/Agent (literal @G-01 or @全体成员).
      // Bracket [skill:]/[source:] are ONLY generated by / slash menu.
      // This hook (useInlineMention) is no longer the @ entry for skills.
      // Keep legacy bracket build only for any stale direct callers (should not happen post S8).
      if (item.type === 'agent') {
        const seq = item.agent?.sequence || item.id
        const mentionText = `@${seq} `
        result = before + mentionText + after
        newCursorPosition = before.length + mentionText.length
      } else {
        // Legacy non-agent path (file/skill etc) — no longer triggered by @ menus.
        // Do not insert bracket here; / path is the generator.
        result = currentValue

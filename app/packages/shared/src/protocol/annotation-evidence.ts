...
import type { ActorRef, DesignSelection, WorkbenchObjectRef } from './design'
...
function annotationId(): string {
  return `ann-${globalThis.crypto?.randomUUID?.() ?? `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`}`
}

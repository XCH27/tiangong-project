/**
 * Fleet 工作台动作信封服务接口（承重墙 · 冻结的服务契约）
 * =====================================================================
 *
 * 这是脊柱层的唯一动作账本接口，不是各工作面的内部编辑引擎。人类 UI 和 AI 工具
 * 产出的 `DesignAction` 都走 `proposeAction` —— 这是"没有第二条审计路径"在工程上
 * 的强制点。真正的内容应用/撤销由各 surface 的原生适配器完成。
 *
 * 规则（实现必须满足）：

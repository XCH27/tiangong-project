...
describe('loadLabelConfig migrations', () => {
  it('backfills identity labels into legacy default workspaces', () => {
    saveLabelConfig(workspaceRoot, {
      ...
      labels: [
        ...
        { id: 'development', name: 'Development', color: { light: '#3B82F6', dark: '#60A5FA' }, children: [{ id: 'code', name: 'Code', color: { light: '#4F46E5', dark: '#818CF8' } }] },
    ...
    expect(config.labels[0]?.id).toBe('identity');
    expect(flat.find(label => label.id === 'leader')?.kind).toBe('identity');
    expect(flat.find(label => label.id === 'content')?.name).toBe('Content');
  });

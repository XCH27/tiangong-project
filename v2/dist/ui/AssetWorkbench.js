...
export function renderAssetPreview(state, region) {
    const t = getTheme(state.theme);
    const note = (() => {
        if (state.runResult?.output_markdown)
            return state.runResult.output_markdown;
    ...
    const md = new Markdown(note, 1, 0, {
        heading: (text) => `${ansiFg(t.accent)}\x1b[1m${text}${ansiReset}`,
        link: (text) => `\x1b[4m${text}\x1b[24m`,
        linkUrl: (text) => `${ansiFg(t.text_l4)}${text}${ansiReset}`,
        code: (text) => `\x1b[7m${text}\x1b[27m`,
        codeBlock: (text) => `${ansiFg(t.text_l2)}${text}${ansiReset}`,
        codeBlockBorder: (text) => `${ansiFg(t.text_l4)}${text}${ansiReset}`,
        quote: (text) => `${ansiFg(t.text_l3)}${text}${ansiReset}`,
        quoteBorder: (text) => `${ansiFg(t.text_l4)}${text}${ansiReset}`,
        hr: (text) => `${ansiFg(t.text_l4)}${text}${ansiReset}`,
        listBullet: (text) => `${ansiFg(t.accent)}${text}${ansiReset}`,
...
export function renderAssetWorkbench(state, region) {
    const t = getTheme(state.theme);

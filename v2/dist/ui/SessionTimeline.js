...
export function renderSessionTimeline(state, region) {
    if (state.stage === "input") {
        return fitLines(renderInputState({
            theme: state.theme,
    ...
    if (state.stage === "plan" && state.meta && state.plan) {
        return fitLines(renderPlanState({
            theme: state.theme,
            meta: state.meta,
            plan: state.plan,
            width: region.width,
    ...
    if (state.stage === "run" || state.stage === "done") {
        return fitLines(renderRunState({
            theme: state.theme,
    ...
    if (state.stage === "fallback" && state.err) {
        return fitLines(renderFallbackState({
            theme: state.theme,
            err: state.err,
            width: region.width,
        }), region);

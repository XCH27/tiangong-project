...
export function renderFooter(state, layout) {
    return [
...
function renderComposer(state, layout) {
...
function renderActionBar(state, layout) {
    const t = getTheme(state.theme);
...
function formatAction(state, label, selected, primary) {
    const t = getTheme(state.theme);
    if (primary) {

...
export function renderInputState({ theme, urlInputRenderedLine, url, skill, skills, doctor, recentTasks, selectedRecentIndex, width, }) {
    ...
    const predict = platform
        ? (platform === "bilibili" || platform === "youtube") ? "将走「官方字幕优先 → 跳过 ASR」"
            : platform === "local" ? "本地文件，无平台风控"
                : platform === "image" ? "截图：只走 VLM 描述，跳过下载/ASR/抽帧"
                    : "可能需要降级通道（Clipper / 上传）"
        : null;
    const lines = [];
    lines.push("");

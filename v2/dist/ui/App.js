import { TUI, matchesKey, ProcessTerminal, Input, } from "@earendil-works/pi-tui";
import { resolveV2Config } from "../config/V2Config.js";
import { THEME_KEYS } from "../theme/theme.js";
import { DEFAULT_SKILLS } from "../mock/data.js";
import { actionSet } from "./ActionModel.js";
...
export class App {
    ...
    chatInput;
    chatHistory = [];
    constructor(tui, transport, config = resolveV2Config()) {
        this.tui = tui;
    ...
    async loadBackendOverview() {
        ...
        if (doctorResult.status === "fulfilled")
            this.doctor = doctorResult.value;
    ...
    startTaskStream(taskRef, fail = "") {
        ...
        this.unsub = this.transport.streamEvents(taskRef.task_id + (fail ? "__" + fail : ""), (ev) => {
            ...
            if (ev.event === "done") {
                ...
                if (this.runId)
                    void this.loadRunResult(this.runId);
                void this.loadBackendOverview();
                this.progress = 1;
                this.mascot = "•‿•";
                this.stage = "done";
    ...
    async executeAction(id) {
        switch (id) {
            ...
            case "use_lighter_route":
                if (await this.switchToTextOnlyRoute())
                    return this.goRun();
                return;
            case "open_recent":
    ...
    async switchToTextOnlyRoute() {
        if (!this.selectSkillByDepth("text_only"))
            return false;
    ...
    async handleComposerSubmit(value) {
    ...
    async setExportDirectory(value) {
...
export function launchApp(transport, config) {

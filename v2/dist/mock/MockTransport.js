...
export class MockTransport {
    async probe(url) { await delay(500); return mockMetadata(url); }
    async plan(url, skill, depth) { await delay(420); return mockPlan(url, skill, depth); }
            ...
            shots: [
                {
                    ...
                    vlm_description: { type: "visual_reference", content: "A terminal UI overview frame.", confidence: 0.82, model: "mock-vlm" },
                },
            ],
            options: { skill: "video-note", depth: "with_frames" },
        };
    }
    async cancelTask(_taskId) { await delay(80); }
    async retryTask(_taskId) {
        await delay(160);
    ...
    async run(emit, isCancelled, fail) {
        let id = 0;
        const e = (event, data) => { if (!isCancelled())
            emit({ id: ++id, event, data }); };
        e("queued", { progress: 0, message: "任务已入队" });
        await delay(450);
        e("probing", { progress: 0.05, step: "probe", message: "探测视频元数据" });
        await delay(650);
        if (fail === "blocked" || fail === "ratelimit") {
            await delay(350);
            const err = fail === "blocked"
            ...
            e("error", { progress: 0.12, error: err });
            return;
        }
        e("subtitle_check", { progress: 0.12, step: "acquire_subtitle", has_official: true, skipping_asr: true, message: "命中官方字幕，跳过 ASR" });
        await delay(750);
        e("segmenting", { progress: 0.28, step: "transcribe", message: "叙事分段" });
        await delay(800);
        e("extracting_frames", { progress: 0.48, step: "extract_frames", message: "按锚点抽取关键帧（每段 ≤3）" });
        await delay(800);
        e("vlm_describing", { progress: 0.62, step: "vlm_describe", message: fail === "oom" ? "本地 VRAM 不足 → 降级云端 VLM" : "VLM 描述关键帧（hypothesis）" });
        await delay(800);
        e("generating", { progress: 0.82, step: "generate", message: "LLM 生成图文笔记" });
        await delay(950);
        e("exporting", { progress: 0.95, step: "export", message: "写出 Markdown 资产" });
        await delay(600);

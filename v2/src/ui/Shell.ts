...
function statusPill(state: ShellRenderState, text: string): string {
  const t = getTheme(state.theme);
  return `${ansiFg(t.text_l4)}[${text}]${ansiReset}`;
}

function renderHeader(state: ShellRenderState, layout: ShellLayout): string[] {
  const t = getTheme(state.theme);
...
function renderMain(state: ShellRenderState, layout: ShellLayout): string[] {
  if (state.activeScreen === "setup" || state.activeScreen === "protocol") return renderCommandSurface(state, layout.main);
  if (state.activeScreen === "chat") return renderAssetPreview(state, layout.main);
  return renderSessionTimeline(state, layout.main);
}

function combineBody(state: ShellRenderState, layout: ShellLayout, rail: string[], main: string[], side: string[]): string[] {
...
export function renderShell(state: ShellRenderState, height = DEFAULT_SHELL_HEIGHT): string[] {
...
export function renderShellFixture(width: number, height = DEFAULT_SHELL_HEIGHT): string[] {
  return renderShell({
    theme: "ink-night",
    stage: "done",
    activeScreen: "chat",
    ...
    progress: 1,
    steps: {
      probe: { step: "probe" as any, status: "completed", message: "ok" },
      export: { step: "export" as any, status: "completed", message: "ok" },
    },
    err: null,
    mascot: "•‿•",
  }, height);

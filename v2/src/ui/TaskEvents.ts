...
import type { PipelineStep, SSEEvent, StepState } from "../types/contracts.js";

const EVENT_TO_STEP: Partial<Record<SSEEvent["event"], { step: PipelineStep; status: StepState["status"] }>> = {
...
export function applyTaskStepEvent(steps: Record<string, StepState>, ev: SSEEvent): Record<string, StepState> {
...
export function completeTaskSteps(steps: Record<string, StepState>): Record<string, StepState> {
  const next = completeRunningSteps(steps);
  next.export = {
    step: "export",
    status: "completed",
...
function completeRunningSteps(steps: Record<string, StepState>): Record<string, StepState> {
  const next = { ...steps };

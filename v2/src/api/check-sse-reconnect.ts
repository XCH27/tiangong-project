...
import type { SSEEvent } from "../types/contracts.js";
...
const server = http.createServer((req, res) => {
  ...
  requests.push(req.headers["last-event-id"] as string | undefined);
...
if (requests[0] !== undefined) throw new Error(`first request should not send Last-Event-ID: ${requests[0]}`);
if (requests[1] !== "2") throw new Error(`reconnect should send Last-Event-ID=2: ${JSON.stringify(requests)}`);

...
- `CliRuntimeCatalog`：detected 映射跟随 AionUi 已验证 ACP smoke（Claude Code / Codex / Goose），并先做本机 PATH 过滤，避免把静态候选误报为“已识别”；custom CRUD（增/改/启停/删）+ 落盘 `~/.craft-agent/.fleet/cli-runtimes.json` + 编辑边界（managed 不可改删、detected 不可改 command 可禁删、custom 全可编辑）。
...
- **动态模型发现与切换**：ACP `session/new` 的 `configOptions` / `models` 归一化为 `CliRuntimeModelState`；切换优先 `session/set_config_option`，兼容 `session/set_model`；不暴露模型时显示“模型由 CLI 管理”。
  - ⚠️ **复审备注（best-effort，非标准 ACP）**：核心 ACP 规范并未标准化模型切换；`session/set_model` 与 `configOptions/session/set_config_option` 是对不同 runtime 的**推测性扩展**。当前实现对不支持的 runtime **安全降级**为“模型由 CLI 管理”（`canSwitch=false`），不会误发命令。真实 Claude/Codex/Goose 是否暴露模型切换能力，必须 opt-in smoke 实机确认；确认前不要把“可切换模型”当作已支持能力对用户承诺。
...
- 真实 Claude/Codex/Goose 的 opt-in smoke（需本机安装 + 登录）。
...
| Claude Code | 本机 PATH 上存在 `claude` 时才出现在 CLI 选择器；选择后发送纯文本，使用 `claude --acp` |
| Codex | 本机 PATH 上存在 `codex` 时才出现在 CLI 选择器；选择后发送纯文本，使用 `codex --acp` |
...
| unsupported detected | Grok/Hermes/OpenCode/Gemini/Qwen 等未进入 AionUi ACP smoke 覆盖的工具不假识别为 detected；如需接入，走 Custom ACP runtime |
...
```bash
FLEET_CLAUDE_SMOKE=1 CLAUDE_BIN=/path/to/claude ./scripts/craft.sh test packages/server-core/src/services/cli-runtime-claude-smoke.test.ts
FLEET_CODEX_SMOKE=1 CODEX_BIN=/path/to/codex ./scripts/craft.sh test packages/server-core/src/services/cli-runtime-codex-smoke.test.ts

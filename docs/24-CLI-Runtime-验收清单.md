...
- **设置页入口**：Settings 增加 `本机 CLI` 页面，刷新自动扫描并自动测试；显示 runtime 列表、protocol、健康状态、命令摘要、已知模型，并支持 Custom ACP runtime 新增/编辑/删除（displayName/command/args/env）。
...
| 刷新设置页 | 扫描 PATH 上常见 Agent CLI，并自动测试每个已发现 runtime |
...
| needs_adapter | native/subscription CLI binary 可启动，但未接发送 adapter，设置页显示“已检测，待 adapter” |
| disabled runtime | 禁用后聊天选择器不可直接选择 |
| native/subscription 防伪发送 | 即使 session 残留选择了 native/subscription runtime，`SessionManager` 也返回中文可操作错误，不按 ACP 启动 |

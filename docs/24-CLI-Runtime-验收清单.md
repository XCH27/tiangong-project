...
## 0 · 当前进度（2026-06-23 · 🟡 候选/待合入，未提交主线）

**已完成（后端基座 + ACP 发送 + 原模型选择器 + 顶部快捷入口）**

- `CliRuntimeCatalog`：detected 映射（Grok/Hermes/OpenCode + Gemini 候选）+ custom CRUD（增/改/启停/删）+ 落盘 `~/.craft-agent/.fleet/cli-runtimes.json` + 编辑边界（managed 不可改删、detected 不可改 command 可禁删、custom 全可编辑）。
...
- 真实 Grok/Hermes/OpenCode 的 opt-in smoke（需本机安装 + 登录）。

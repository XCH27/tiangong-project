...
| CTX-2.5 ContextCenter Overview | 只读聚合 Usage、Project Environment、context 类工具、可选 ProjectPack summary / dry-run preview；输出 review readiness（ready / blocked / needs_pack / unknown）；RPC 入参有运行时校验；不外发 | `shared/protocol/context-center.ts`、`server-core/services/context-center-service.ts`、`contextCenter:getOverview` |

...
| CTX-2 ProjectPack v1 | 打包当前项目/当前 diff，生成 token count、文件清单、secret scan 和 review prompt | `server-core/services/project-pack-*`、Fleet 数据目录 |
| CTX-2.5 ContextCenter Overview | 只读聚合 Usage、Project Environment、context 类工具、可选 ProjectPack summary；不触发打包、不写 bundle、不外发 | `shared/protocol/context-center.ts`、`server-core/services/context-center-service.ts`、`contextCenter:getOverview` |

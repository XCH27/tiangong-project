...
| CTX-2 ProjectPack v1 | 打包当前项目/当前 diff，生成 token count、文件清单、secret scan 和 review prompt；已支持 dry-run plan preview，不写 bundle | `shared/protocol/project-pack.ts`、`server-core/services/project-pack-*`、`projectPack:previewPlan/pack/getSummary` |
| CTX-2.5 ContextCenter Overview | 只读聚合 Usage、Project Environment、context 类工具、可选 ProjectPack summary / dry-run preview；输出 review readiness（ready / blocked / needs_pack / unknown）；不外发 | `shared/protocol/context-center.ts`、`server-core/services/context-center-service.ts`、`contextCenter:getOverview` |
| CTX-3 MarkItDown sidecar | 允许把 PDF/PPT/Office/HTML 转 Markdown 后纳入 pack | optional sidecar adapter |

...
At session end, distilling tool invocation history (Track A) into structured Fact entries (Track B) is a background, non-interactive operation. This is a classic use case for Craft Agents (二开补强)'s internal `async-native` batch mode:
...
This section defines how Craft Agents (二开补强) reduces token consumption across the request lifecycle.  
...
4. **Context-mode (ELv2) code is strictly forbidden.** Output folding criteria can be researched and independently re-implemented in the Craft Agents (二开补强) UI layer, but no source code, type definitions, or configuration from Context-mode may enter the repository.
...
- **Action**: Independent MCP Server. Model calls `query_code_graph` and `query_vector_index` tools. Craft Agents (二开补强) does not introduce their internal implementations.
- **Upgrade Path**: The MCP protocol ensures engine upgrades do not require Craft Agents (二开补强) modifications.

#### AST Structural Outline — Replacing Repomix Copies (Layer 1)
- **Strategy**: `Repomix` is a candidate reference, and its policy rules explicitly prohibit copying AST parsers, line counters, or ignore-file readers. Extracting its Tree-sitter algorithm to develop a proprietary package for Craft Agents (二开补强) is a **policy violation**.
...
- **Correct Way**: Reference its reversible hash cache interface spec to implement a cache backend inside Craft Agents (二开补强)'s existing Cost Ledger (M11). The hash key is a stable prompt prefix hash generated by Layer 2.

#### Mem0 + Letta (Layer 0 — Memory)
- **Strategy**: Do not copy backend storage wrappers or Qdrant/Milvus interfaces.
- **Correct Way**: Adopt a three-tier memory model (Core / Recall / Archival) at the interface layer. The storage backend is Craft Agents (二开补强)'s native SQLite + FTS5 (utilizing Zvec's indexing patterns). The `DistilledToolMemory` schema in §8.2 is a concrete implementation of the "Recall" tier.
...
In Craft Agents (二开补强), every agent seat carries identity tags (AgentRole / AgentSeat), which bind with Tool Memory:

...
| DWF-1 右侧浏览器设计模式入口 | `BrowserPane` 停靠在主聊天右侧（不是新窗口）；浏览器 toolbar 直接显示“选择/框选/标注/Comment AI/保存证据”模式 | 当前分支未实现；复用 craft BrowserPane/CDP，旧工作树 diff 仅作候选 |
| DWF-2 多选/框选证据包 | 框选多个元素，生成截图、boxes、accessibility 摘要、DOM 摘要 | 当前分支未实现 |
| DWF-3 批量注释与 Comment AI | 对多个元素一起写批注，发送给 Agent 时自动带选区证据 | 后端 `DesignAction(kind:'annotate') -> DesignAnnotationApplier -> AnnotationV1` 已接现有 annotation 写入/回滚；下一刀接浏览器 toolbar 与 Comment AI prompt |
| DWF-4 动作信封与原生引擎 | UI 和 AI 使用同一结构化工具/动作信封，经工作面原生引擎写入 timeline/permission/rollback | 当前分支已有基础 DesignAction/DesignEngine；具体设计文档模型与 applier 尚未确定，按 `docs/30`/`31` 落地 |
...
下一步应保持干净 Craft 默认界面，先完成团队脊柱和全部文件/Library，再把设计能力放到独立无限画布工作面跑通。人和 Agent 的写操作共用原生引擎、permission 和 session timeline，不新建 `WorkbenchShell` 或万能 patch 引擎。

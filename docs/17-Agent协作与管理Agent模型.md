...
> 当前实现状态：`app/packages/server-core/src/services/agent-lifecycle.ts` 已有 Agent 生命周期纯服务，`decision-service.ts` 已有 L0-L3 自动决策规则和 audit 纯服务。它们尚未接 RPC/UI/permission/timeline；后续接入时不得绕过 craft permission，自动代答必须把依据写入 timeline。

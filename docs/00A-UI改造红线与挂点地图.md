...
| 记忆模块 | ✅ 已落：`pages/settings/MemorySettingsPage.tsx`（craft 设置骨架），真接 `memory` RPC。按 user/software/project/agent/task/design_asset/external_review 分区查看、添加、删除；scoped 分区按 scopeId 隔离。入口是设置里的「记忆」，不是管理 Agent 页的附属区 | 别和管理 Agent 自动决策混成一个页面；别建第二套 memory store；别做没有 scope 隔离的项目记忆 |

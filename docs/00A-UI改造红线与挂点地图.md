...
| 全部文件 / Library | ✅ raw「全部文件」只读入口已落：左栏与「所有会话」同级，`files` navigator + `renderer/components/files/FilesListPanel.tsx`，后端复用文件 RPC 并新增只读 `fs:listEntries`，中栏浏览当前工作区文件/文件夹，右侧复用 `FileViewer` 预览文本文件。⏳ 未落：用户额外素材目录、类型过滤、缩略图、拖入专业工作面、Library 资产层、文件整理写操作（这些必须先接 permission + timeline） | 别和「本地知识库」数据源混成一个概念；别给 raw view 加未授权的移动/删除/重命名假按钮 |
